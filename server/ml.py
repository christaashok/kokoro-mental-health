...
def demo():
    """Demo function to test the ML service"""
    ...
    for text in sample_texts:
        ...
        result = recommender.get_emotion_playlist(text, min_popularity=30, limit=20)
        
        if result["success"]:
            emotion_data = result["emotion_analysis"]
            music_data = result["music_recommendations"]
            
            print(f"😊 Emotion: {emotion_data['emotion'].upper()} ({emotion_data['confidence']:.2f} confidence)")
            
            if music_data["success"]:
                print(f"🎶 Found {len(music_data['tracks'])} recommendations:")
                print(f"📊 Average popularity: {music_data['metadata']['avg_popularity']}/100")
                
                for i, track in enumerate(music_data['tracks'], 1):
                    print(f"  {i}. {track['name']} by {track['artist']} (⭐ {track['popularity']}/100)")
            else:
                print(f"❌ Music recommendation failed: {music_data['error']}")
        else:
